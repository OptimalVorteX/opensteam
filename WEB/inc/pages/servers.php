<?php
  ...
  if ( OSS_SuperAdmin()  ) {
	 ...
	 if(isset($_GET["remove"]) AND is_numeric($_GET["remove"]) ) {
		...
		$result = $del->execute();
		header("location: ".OSS_HOME."?option=servers");
		die();
	 }
	 ...
	 if (isset($_POST["server_add"]))  {
		...
		if( strlen($server_name)<=2 OR strlen($server_ip)<=2 OR empty($server_port) OR !is_numeric($server_port) ) {
		   header("location: ".OSS_HOME."?option=servers&error");
		   die();
		}
		//INSERT/UPDATE
        if(!isset($_GET["edit"])) {
        ...
        if(isset($_GET["add"])) header("location: ".OSS_HOME."?option=servers"); else
	 	header("location: ".OSS_HOME."?option=servers&edit=".$serverID);
	    die();
	 }
	 ...
	 if(isset($_GET["com"]) AND isset($ServersData[0]["server_ip"]) ) {
		...
		header("location: ".OSS_HOME."?option=servers&rcon=".$ServersData[0]["id"]."&command=".$command);
		die();
	 }
