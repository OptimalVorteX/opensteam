     ...
     <table class="table table-striped table-hover ">
	  ...
	  <tr>
		  ...
		  <td>
		    ...
		    <a href="<?=OSS_HOME?>?option=servers" class="btn btn-danger"><?=$lang["Cancel"]?></a>
		  </td>
	  </tr>
	 </table>
   </form>
...
<h2><?=$lang["Servers"]?> <a href="<?=OSS_HOME?>?option=servers&amp;add" class="btn btn-primary"><?=$lang["AddServer"]?></a></h2>
<table class="table table-striped table-hover ">
  <thead>
    <tr>
      <th><?=$lang["Server"]?></th>
  ...
  ?>
   <tr>
      <td width="320"><a class="btn btn-info btn-xs" href="<?=OSS_HOME?>?option=servers&amp;rcon=<?=$server["id"]?>"><span><?=$server["server_name"]?></span></a></td>
	  ...
	  <td> 
	    <a href="<?=OSS_HOME?>?option=servers&amp;edit=<?=$server["id"]?>" class="btn btn-info btn-xs"><?=$lang["Edit"]?></a>
		<a onclick="if(confirm('Delete Server?')) { location.href='<?=OSS_HOME?>?option=servers&amp;remove=<?=$server["id"]?>' } " href="javascript:;" class="btn btn-danger btn-xs"><?=$lang["Remove"]?></a>
	  </td>
   </tr>
  <?php } ?>
  </tbody>
...
<div class="well bs-component">
   ...
   <form action="" method="post">
		 ...
		 <a href="<?=OSS_HOME?>?option=servers" class="btn btn-danger btn-xs">&laquo; <?=$lang["Back"] ?></a>
   </form>
...
<div class="well bs-component">
   ...
   $c++; ?>
    <tr> 
		...
		<a onclick="if(confirm('Kick [<?=$player["Name"]?>]')) { location.href='<?=OSS_HOME?>?option=servers&amp;rcon=<?=$ServersData[0]["id"]?>&amp;com=ulx kick <?=$player["Name"]?>' }" href="javascript:;" class="btn btn-danger btn-xs"><?=$lang["kick"] ?></a>
		<?php } ?>
		</td>
	</tr>
   <?php } ?>
